...
import { RestaurantsResponse, SingleRestaurantResponse } from '../interfaces/response';
...
export class RestaurantsService {
  ...
  getRestaurant(id: number): Observable<Restaurant>{
    return this.#http.get<SingleRestaurantResponse>(
      `${this.#restaurantUrl}/${id}`
    ).pipe(map((r)=>r.restaurant));
  }
  addRestaurant(restaurant: RestaurantInsert): Observable<Restaurant>{
    return this.#http.post<SingleRestaurantResponse>(
      this.#restaurantUrl, restaurant
